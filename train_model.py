...
from text_xtractor import Textxtractor
...
def train_xtractor():
    ...
    # Build a graph and xtractor object
        ...
        with sess.as_default():
            ...
            with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
                learning_rate = tf.train.exponential_decay(learning_rate=args.learning_rate,
                                                           global_step=xtractor.global_step,
                                                           decay_steps=args.decay_steps,
                                                           decay_rate=args.decay_rate,
                                                           staircase=True)
                optimizer = tf.train.AdamOptimizer(learning_rate)
                grads, vars = zip(*optimizer.compute_gradients(xtractor.loss))
                grads, _ = tf.clip_by_global_norm(grads, clip_norm=args.norm_ratio)
                train_op = optimizer.apply_gradients(zip(grads, vars), global_step=xtractor.global_step, name="train_op")
            ...
            # Summaries for loss
            loss_summary = tf.summary.scalar("loss", xtractor.loss)
            ...
            current_step = sess.run(xtractor.global_step)
            ...
            def train_step(batch_data):
                ...
                feed_dict = {
                    xtractor.input_x: x,
                    xtractor.input_y_first: sec,
                    xtractor.input_y_second: subsec,
                    xtractor.input_y_third: group,
                    xtractor.input_y_fourth: subgroup,
                    xtractor.input_y: y_onehot,
                    xtractor.dropout_keep_prob: args.dropout_rate,
                    xtractor.alpha: args.alpha,
                    xtractor.is_training: True
                }
                _, step, summaries, loss = sess.run(
                    [train_op, xtractor.global_step, train_summary_op, xtractor.loss], feed_dict)
            ...
            def validation_step(val_loader, writer=None):
                ...
                for batch_validation in batches_validation:
                    ...
                    feed_dict = {
                        ...
                        xtractor.input_y_fourth: subgroup,
                        xtractor.input_y: y_onehot,
                        xtractor.dropout_keep_prob: 1.0,
                        xtractor.alpha: args.alpha,
                        xtractor.is_training: False
                    }
                    step, summaries, scores, cur_loss = sess.run(
                        [xtractor.global_step, validation_summary_op, xtractor.scores, xtractor.loss], feed_dict)
            ...
            for batch_train in batches_train:
                ...
                current_step = tf.train.global_step(sess, xtractor.global_step)
